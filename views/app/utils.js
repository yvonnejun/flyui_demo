...
Utils.prototype = {
    ...
    isSoldierCard: function (card) { //匹配红头文件排头
     ...
     * @return {string}
     */
    getNowFormatDate: function (param) { //获取当前日期拼成字符串，往往是后台需要的时间参数格式
        var date = new Date();
     ...
     * @return {string}
     */
    getNowFormatTime: function (param) { //获取当前时间拼成字符串
        var date = new Date();
        var seperator1 = param ? param : ':';
    ...
    getDateAddDayCount: function (AddDayCount) { //获取当前时间多少AddDayCount天后的日期
        var date = new Date();
    ...
    /**
     * 获取当前时间
     * @param  {format}  param  yyyyMMddHHmmss   //获取现在当前时间，转换成年月日1
     */
    ...
    calcdate: function (time) { //获取现在当前时间，转换成年月日2
        ...
        timeNow = llqUtils.timeForData(timeNow); //将日期转为字符串
    ...
    compareTime: function (startTime, endTime) { //航班时间比较
